...
# from flight_reservation.config.database_config import get_db_connection
...
class ReservationController:
    ...
    def view_reservations(self, user):
        ...
        if "admin" in user.roles:
            reservations = self.reservation_repo.get_all_reservations()
        else:
            reservations = self.reservation_repo.get_reservations_by_user(user)
    ...
    def cancel_reservation(self, user):
        ...
        reservation_number = int(input("Please enter the reservation number you want to cancel: "))
        ...
        # extract just the reservation_number from each record
        # if your repo returns tuples like (res_id, user_id, ...):
        res_ids = [r[0] for r in user_reservations]

        if reservation_number not in res_ids:
    ...
    def make_reservation(self, user, direct_flight=None, itinerary=None):
        ...
        try:
            ...
            print("❌ Please enter a valid integer for seats.")
            return

        # Build a flat list of Flight objects to book
        if direct_flight:
            flights_to_book = [direct_flight]
        elif itinerary:
            flights_to_book = []
            for elem in itinerary:
                if isinstance(elem, list):
                    # this is your nested leg-list
                    flights_to_book.extend(elem)
                else:
                    flights_to_book.append(elem)
        else:
            print("❌ No flight or itinerary selected.")
            return
        # print("Initiating reservation process...")
        # try:
        #     num_seats = int(input("How many seats would you like to reserve?: "))
        ...
        #     return
        #
        # # Determine which set of flights we're booking
        # if direct_flight:
        #     flights_to_book = [direct_flight]
        # elif itinerary:
        #     flights_to_book = itinerary
        # else:
        #     print("❌ No flight or itinerary selected.")
        #     return

        # Now all elements of flights_to_book are Flight instances
        available_seats = min(f.get_available_seats() for f in flights_to_book)
        if num_seats < 1 or num_seats > available_seats:
            print(f"❌ Cannot reserve {num_seats} seats. Only {available_seats} available.")
            return
        ...
        total_price = sum(float(f.get_ticket_price()) * num_seats for f in flights_to_book)
        print(f"Total price for {num_seats} seat(s): ${total_price:.2f}")

        # 3) Persist each leg via the reservation_repo
        now = datetime.now()
        for f in flights_to_book:
            self.reservation_repo.create_reservation(
                user=user,
                flight_no=f.get_flight_no(),
                seats=num_seats,
                creation_date=now,
                payment_amount=float(f.get_ticket_price()) * num_seats
            )
            # 4) (Optional) update the flight’s booked_seats in DB
            # self.flight_repo.update_booked_seats(f.get_flight_no(), num_seats)
    ...
    #         # Handles both direct flights and multi-leg itineraries
    #         if isinstance(flight, list):
    #             available_seats = min(f.get_available_seats() for f in flight)
    #         else:
